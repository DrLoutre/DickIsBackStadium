...
class Runners {
  ...
  var idList:LinkedList[String] = new util.LinkedList[String]()
  var idNumber:LinkedList[Integer] = new util.LinkedList[Integer]()
  var time:LinkedList[LinkedList[Long]] = new util.LinkedList[util.LinkedList[Long]]()

  def scanned(id: String):Unit = {
    if (idList.contains(id)) {
      val index = idList.indexOf(id)
      val newVal = idNumber.get(index)
      val perfs: LinkedList[Long] = time.get(index)
